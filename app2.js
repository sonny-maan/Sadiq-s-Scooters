...
function create() {
  ...
  let options = {
    location: new Location(Math.random(), Math.random()),
    destination: new Location(Math.random(), Math.random()),
    path: [],
    // path: path,
...
function drawPeople() {
  ...
  world.people.forEach(person1 => {
    if (person1.onVehicle) {
      context.fillRect(person1.location.x * canvas.width, person1.location.y * canvas.height, width + 20, height);
    } else {
      context.fillRect(person1.location.x * canvas.width, person1.location.y * canvas.height, width, height);
    }
    ...
    context.beginPath();
    context.moveTo(person1.location.x * canvas.width, person1.location.y * canvas.height);
    context.lineTo(person1.destination.x * canvas.width, person1.destination.y * canvas.height);
    context.stroke();
    ...
    if (person1.path[0]) {
      ...
      context.moveTo(person1.location.x * canvas.width, person1.location.y * canvas.height);
      context.lineTo(person1.path[0].x * canvas.width, person1.path[0].y * canvas.height);
      context.stroke();
      context.strokeStyle = 'blue'
      context.beginPath();
      context.moveTo(person1.destination.x * canvas.width, person1.destination.y * canvas.height);
      context.lineTo(person1.path[0].x * canvas.width, person1.path[0].y * canvas.height);
      context.stroke();
    }
  ...
  world.dockingStations.forEach(dockingStation => {
    let eh1 = dockingStation._location.x
    let eh2 = dockingStation._location.y
